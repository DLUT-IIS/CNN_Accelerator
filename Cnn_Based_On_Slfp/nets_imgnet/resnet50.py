...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if self.pre_reference == True or qbit == 32:
            ...
            # Conv2d_1 = conv2d_Q_bitch_size_scaling(q_bit = qbit, Kw = Kw[1], Ka = Ka[1])
            # Conv2d_2 = conv2d_Q_bitch_size_scaling(q_bit = qbit, Kw = Kw[2], Ka = Ka[2])
            # Conv2d_3 = conv2d_Q_bitch_size_scaling(q_bit = qbit, Kw = Kw[3], Ka = Ka[3])
            """--------------- choose layer scale----------------"""
            Conv2d_1 = conv2d_Q(q_bit = qbit, Kw = Kw[1], Ka = Ka[1])
            Conv2d_2 = conv2d_Q(q_bit = qbit, Kw = Kw[2], Ka = Ka[2])
            Conv2d_3 = conv2d_Q(q_bit = qbit, Kw = Kw[3], Ka = Ka[3])
...
class ResNet50(nn.Module):  
    def __init__(
        ...
        if self.pre_reference == True or qbit == 32:
            ...
            # kw = acquire_weight_bitch_size_scale_factor_txt('/workspaces/pytorch-dev/Cnn_Based_On_Slfp/max_act_wgt/imgnet/resnet_layer_weight_max.txt')   
            # Kw = [[x / MAXIMUM_MAGNITUDE for x in sub_list] if isinstance(sub_list, list) else sub_list / MAXIMUM_MAGNITUDE for sub_list in kw]

            # Conv2d_first = conv2d_Q_bitch_size_scaling(q_bit = qbit, Kw = Kw[0], Ka = Ka[0])
            # Linear = linear_Q_bitch_size_scaling(q_bit = qbit, Kw = Kw[53], Ka = Ka[53])
    ...
    def _make_layer(
        ...
        if self.pre_reference == True or self.qbit == 32:
            ...
            """--------------- choose bitch size scale----------------"""
            # conv1x1_downsample = conv2d_Q_bitch_size_scaling(q_bit = self.qbit, Kw = Kw[0], Ka = Ka[0])
            """--------------- choose layer scale----------------"""
            conv1x1_downsample = conv2d_Q(q_bit = self.qbit, Kw = Kw[0], Ka = Ka[0])           
