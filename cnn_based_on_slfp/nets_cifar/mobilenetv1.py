...
class MobileNetV1_Q(nn.Module):
    def __init__(self, ch_in, qbit, pre_reference):
        ...
        if self.pre_reference == True or qbit == 32:
                ...
                kw = acquire_weight_scale_factor_txt('/workspaces/pytorch-dev/Cnn_Based_On_Slfp/max_act_wgt/cifar/batch_size_weight_max.txt')   
                Kw = [[x / MAXIMUM_MAGNITUDE for x in sub_list] if isinstance(sub_list, list) else sub_list / MAXIMUM_MAGNITUDE for sub_list in kw]
                        
                Conv2d = conv2d_Q_batch_size_scaling(q_bit = qbit, Kw = Kw, Ka = Ka)
                Linear = linear_Q_batch_size_scaling(q_bit = qbit, Kw = Kw[27], Ka = Ka[27]) 
                ...
                kw = acquire_weight_scale_factor_txt_improved('/workspaces/pytorch-dev/Cnn_Based_On_Slfp/max_act_wgt/cifar/batch_size_weight_max_improved.txt')   
                kw_first_layer = kw[0]
                kw_other       = kw[1:]
