...
def get_scale_factor_moebz_improved(model, data_loader, total_images): # get Ka, Kw
    ''' Function:   maximum of every batch sizes, 
    ...
    for idx, inputs_list in layer_inputs.items():
        max_abs_input = torch.max(torch.abs(torch.cat(inputs_list, dim=0))) # torch.cat ：merge batch size 
...
def get_scale_factor_moabz(model, data_loader, total_images): # get Ka, Kw
    ...
    for idx, inputs_list in layer_inputs.items():
        max_abs_input = torch.max(torch.abs(torch.cat(inputs_list, dim=0))) # torch.cat ：合并 batch size 
        max_abs_layer_inputs.append(max_abs_input.item()) 
...
def put_scale_factor_batch_size_txt(max_abs_layer_inputs_list, max_abs_layer_outputs_list, max_abs_channel_weights, weight_folder_path, input_folder_path):
...
def acquire_weight_batch_size_scale_factor_txt(folder_path):
