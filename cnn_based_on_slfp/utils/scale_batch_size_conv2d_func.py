...
# ------------------------------------ batch_size_scale---------------------------------------- #
"""  
    Function: Batch_size_scaling, Each convolution kernel uses a scaling factor.
    Author  : Zhangshize
    Bias    : False
"""
def conv2d_Q_batch_size_scaling(q_bit, Kw, Ka):
  class conv2d_Q_batch_size_scaling(nn.Conv2d):  
    def __init__(self,in_channels, out_channels, kernel_size, Kw = Kw, Ka = Ka, stride=1, padding=0, dilation=1, groups=1, bias=False):
      super(conv2d_Q_batch_size_scaling, self).__init__(in_channels, out_channels, kernel_size, stride,
                                    padding, dilation, groups, bias)
      self.q_bit = q_bit
      self.quantize_weight = weight_quantize_func(q_bit=q_bit)
      self.quantize_act = act_quantize_func(q_bit=q_bit)
  ...
  return conv2d_Q_batch_size_scaling

"""  
  Function: Batch_size_scaling, Each Fully connected group uses a scaling factor.
  Author  : Zhangshize
  Bias    : False
"""
def linear_Q_batch_size_scaling(q_bit, Kw, Ka):   
  class linear_Q_batch_size_scaling(nn.Linear):
    def __init__(self, in_features, out_features, Kw=Kw, Ka=Ka, bias = False):
      super(linear_Q_batch_size_scaling, self).__init__(in_features, out_features, bias )
      self.q_bit = q_bit
      self.quantize_weight = weight_quantize_func(q_bit=q_bit)
      self.quantize_act = act_quantize_func(q_bit=q_bit)
    ...
    def forward(self, input):
      ...
      return final_output
  return linear_Q_batch_size_scaling
...
"""  
    ...
    Bias    : True
"""
def conv2d_Q_bias_batch_size_scaling(q_bit, Kw, Ka):
  class conv2d_Q_bias_batch_size_scaling(nn.Conv2d):  
    def __init__(self,in_channels, out_channels, kernel_size, Kw = Kw, Ka = Ka, stride=1, padding=0, dilation=1, groups=1, bias=True):
      super(conv2d_Q_bias_batch_size_scaling, self).__init__(in_channels, out_channels, kernel_size, stride,
                                    padding, dilation, groups, bias)
      self.q_bit = q_bit
      self.quantize_weight = weight_quantize_func(q_bit=q_bit)
      self.quantize_act = act_quantize_func(q_bit=q_bit)
    ...
    def forward(self, input, order=None):
      """ Each batch size is scaled """
      for index, batch_size_weight_max in enumerate(self.Kw):
          batch_size_weight_q = torch.div(self.weight[index] , torch.tensor(batch_size_weight_max)) # tentor division
          if index == 0:
            layer_weight_q = batch_size_weight_q
            layer_weight_q = layer_weight_q.unsqueeze(0)
          else:
            batch_size_weight_q = batch_size_weight_q.unsqueeze(0)
            layer_weight_q = torch.cat((layer_weight_q, batch_size_weight_q), dim=0)
  ...
  return conv2d_Q_bias_batch_size_scaling
...
"""  
  ...
  Bias    : True
"""
def linear_Q_Bise_batch_size_scaling(q_bit, Kw, Ka):   
  class linear_Q_Bise_batch_size_scaling(nn.Linear):
    def __init__(self, in_features, out_features, Kw=Kw, Ka=Ka, bias=True):
      super(linear_Q_Bise_batch_size_scaling, self).__init__(in_features, out_features, bias)
      self.q_bit = q_bit
      self.quantize_weight = weight_quantize_func(q_bit=q_bit)
      self.quantize_act = act_quantize_func(q_bit=q_bit)
    ...
    def forward(self, input):
      """ Each batch size is scaled """
      for index, batch_size_weight_max in enumerate(self.Kw):
          print('Batch size index of the layer ',index)
      ...
      return final_output
  return linear_Q_Bise_batch_size_scaling
